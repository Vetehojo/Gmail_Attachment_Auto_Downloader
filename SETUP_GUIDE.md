...
1. `setup.bat` を実行する。Python 3.14を確認し、依存パッケージを `requirements.lock` の固定バージョンで導入したあと（下記「依存パッケージ」）、設定GUIが開く（設定内容は下記のOAuthモード / DWDモードを参照）。トレイアプリが動いている間は設定GUIを開かないので、トレイメニューの「設定」を使う
...
- どちらもバッテリー駆動中でも起動し、電源ケーブルを外しても止まらない
...
| ファイル | 用途 |
| --- | --- |
| `setup.bat` | 依存導入（`requirements.lock`）+ 設定GUI + お試し取得（任意）。自動取得は開始しない |
