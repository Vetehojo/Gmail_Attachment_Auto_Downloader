...
- インストール先のフォルダーか、Python（`pythonw.exe`）のパスに、Windowsの文字コード（日本語版ではShift_JIS）で表せない文字（絵文字など）があると登録できない。表せる文字だけの場所に置き直し、タスクスケジューラに上の2つのタスクが残っていれば削除してから、もう一度実行する
